...
Server.prototype.sendServerData = function(id, code, content, parameters) {
	...
	this.sendData(id, 
		code + ' ' + ((this.users[id].nick !== undefined) ? this.users[id].nick : this.users[id].addr) + 
		((parameters !== undefined) ? ' ' + parameters : '') + ((content !== undefined) ? ' :' . content : ''), 'server');
};
...
Server.prototype.broadcastOnChannel = function(channel, cmd, ignore, type, source) {
	...
	for(var i in this.channels[channel].users) {
		if(ignore.indexOf(this.channels[channel].users[i]) == -1)
			this.sendData(this.channels[channel].users[i], cmd, type, source);
	}
};
